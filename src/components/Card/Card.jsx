...
function Card({ product }) {
  ...
  const handleAddToCart = async (e) => {
    ...
    if (isAuth) {
      if (isCart) {
        ...
        if (isEmpty) {
          cartItemsRef.doc(product.supplier.id.toString()).delete();
        }
      } else {
        setIsCart(true);

        const isEmpty = (
          await cartItemsRef.doc(product.supplier.id.toString()).get()
        ).exists;

        if (!isEmpty) {
          cartItemsRef.doc(product.supplier.id.toString()).set(oneShop);
        }
        cartItemsRef
          .doc(product.supplier.id.toString())
          .collection("cart")
  ...
  const handleToggleFavs = async (e) => {
    ...
    if (isAuth) {
      if (isFavs) {
        setIsFavs(false);
        favItemsRef.doc(product.id.toString()).delete();
      } else {
        setIsFavs(true);
          ...
          <div className="card-info">
            ...
            <div className="card-info__btns">
              <button className="card-info__btns-buy">
                {!product.costSale ? (
                  ...
                  <div className="flex gap-x-2 justify-center items-baseline">
                    <span className="line-through text-sm text-greyf2">
                      {product.cost} с
                    </span>
              ...
              <button
                ...
                className="card-info__btns-fav"
              >
                <img src={isFavs ? AddToFavoriteFill : AddToFavorite} />
              </button>
              <button
                onClick={handleAddToCart}
                className="card-info__btns-card"
              >
                <img
                  className="sm:hidden"
                  src={isCart ? AddToCartFill : AddToCart}
                  alt="agrigram-cart"
                />
                <img
                  className="hidden sm:block"
                  src={isCart ? AddToCartPlusAct : AddToCartPlus}
                  alt="agrigram-cart"
                />
              </button>
            </div>
