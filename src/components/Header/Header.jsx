...
import Logo from "../../assets/imgs/header-icons/logo.svg";
...
function Header() {
	...
	useEffect(() => {
		setIsLang(options.find(opt => opt.value == localStorage.getItem("i18nextLng")));
	}, [])
  ...
  return (
		<>
			...
			<header className="header">
				...
				<div onClick={() => setIsActiveMenu(false)} className={`menu-block ${isActiveMenu ? "opacity-80" : "opacity-0 pointer-events-none"} `}/>
				...
				<nav className="header-menu">	
					...
					<div className="hidden md:flex items-center">
						<div className="relative max-w-[1.5rem] h-5">
							<div onClick={onClickIsActiveMenu} className="burger">
								<div className={`${isActiveMenu ? "menu-burger-fixed" : ""}`}>
									...
									<span className={`span-tree ${isActiveMenu && "active"}`}></span>
								</div>
							</div>
								...
								<div className="mobile-menu-items">
									{navMenu.map(({title, path}, idx) => 
										<Link
											...
											onClick={() => {handleClickIsActive(idx), onClickIsActiveMenu()}}
											key={idx}
											to={path}>
												{headerTr[title]}
										</Link>
						...
						<div className={`header-menu-logo ${isActiveMenu ? "menu-logo-fixed" : ""}`}>
							<Link to="/"> 
								<img 
									onClick={() => {setIsActiveMenu(false), setIsActive(null)}}
									src={MobLogo} alt="kurjun"
								/> 
							</Link>
						</div>
					</div>

					<Link className="header-menu-item" to="stores">{headerTr.stores}</Link>
					<div className="relative" onClick={() => setVisibleMenu(false)}
						onMouseOver={() => {setVisibleMenu(true)}}
						onMouseOut={() => {setVisibleMenu(false)}} 
					>
						<Link className="header-menu-item" to="category">
							{headerTr.categories}
							<img className={`transition-all ${visibleMenu ? "" : "-rotate-90"}`} src={ArrowIcon} />
						</Link>
						<CategoryNavLinks visibleMenu={visibleMenu} setVisibleMenu={setVisibleMenu} />
					</div>
					<Link to="contacts" className="header-menu-item">{headerTr.contacts}</Link>
					<Link to="about-us" className="header-menu-item">{headerTr.aboutus}</Link>
				</nav>

				<Link to="/search-product">
					<div className="header-search search">
						<img className="search-icon" src={SearchIcon} />
						<input disabled className="search-input" type="text" placeholder={headerTr.search} />
					</div>
				</Link>

				<div className="header-icons">
					<Link to={isAuth ? "/wish-list" : "/auth"}>
						<img src={FavoriteIcon} alt="kurjun-favorite" />
					</Link>
					<Link to={isAuth ? "/cart" : "/auth"}>
						<img src={CartIcon} alt="kurjun-cart" />
					</Link>
					<Link to="/profile/my-data" >
						<img src={Profile} alt="kurjun-profile" />
					</Link>
					<Select
						className="sm:hidden"
						options={options}
						...
						getOptionLabel={(option) => (
							<div className="flex gap-x-3 justify-center items-center">
								<img className="w-5 h-5 object-contain" src={option.image} alt={option.label} />
					...
					<nav className="burger-menu-right sm:hidden">
						...
						<div className={`burger-menu-right-block ${isActiveMenu && "active"}`}>
							<div>
								{navMenu.map(({title, path}, idx) => 
									<Link className={`${isActive === idx ? "active" : "" }`} 
										onClick={() => {handleClickIsActive(idx), onClickIsActiveMenu()}}
										key={idx} to={path}>{headerTr[title]}
									</Link>
								)}
							</div>
						</div>
