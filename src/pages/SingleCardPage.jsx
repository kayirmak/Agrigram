...
import { getOneProduct, getSimilarProducts } from '../store/products/productApi';
...
function SingleCardPage() {
  ...
  const [isModal, setIsModal] = useState(false);
	...
	useEffect(() => {
    getOneProduct(dispatch, productId)
      .then(currProd => {
  ...
  useEffect(() => {
    if (isAuth) {
      ...
      setIsCart(!!cartItemsByUser.find(store => {
    ...
    if (isAuth) {
      ...
      .then(async (items) => {
        const cartList = items.docs.map(async (item) => {
          ...
          return {...item.data(), items: cartList.docs.map(item => item.data())};
        });
        Promise.all(cartList).then(cartList => dispatch(getCartItemsSuccess(cartList)))
      })
    }
  };
    ...
    <div className="single-card-page">
      <div className="single-card">
        ...
        <>
            ...
            >
              <SwiperSlide>
                <div className="swiper-zoom-container single-card-imgs-main h-[37rem] rounded-xl">
                    <ImageLoad src={activeImg} alt="kurjun-card-main-img" className="w-[100%!important] h-[100%!important] !object-contain !important rounded-xl" />
                </div>
              </SwiperSlide>
            </Swiper>
          </div>
          ...
          <div className="hidden sm:block max-w-[37rem] xb:max-w-[25rem] w-full mx-auto px-[3rem] xb:px-[1rem]">
            <Swiper pagination={{el: ".swiper-pagination", clickable: true}}  modules={[Pagination]} className="mySwiper swiper-block">

              <SwiperSlide>
                <div className="h-[30rem] xb:h-[20rem]">
                  <img className="w-full h-full object-contain" src={currentProduct.image} alt="image" />
                </div>
              </SwiperSlide>
        ...
        <div className="single-card-details">
          <div className="flex justify-between">
            <h2 className="single-card-details__title">{currentProduct.name}</h2>
            <button onClick={handleToggleFavs} className="single-card-details__btns-fav fav"><img src={isFavs ? AddToFavFill : AddToFav} /></button>
          </div>
          <div className="single-card-details__btns sm:hidden">
            {!isCart ? 
              <button onClick={handleAddToCart} className="single-card-details__btns-cart">{SingleCardPageTr.addCard}</button>
              :
              <Link to="/cart" className="single-card-details__btns-link">{SingleCardPageTr.gotoCard}</Link>
            }   
            <button onClick={handleToggleFavs} className="single-card-details__btns-fav"><img src={isFavs ? AddToFavFill : AddToFav} /></button>
          </div>
          ...
          <p className="single-card-details__des">{currentProduct.description}</p>
      ...
      <div className="single-card-info">
        <div className="single-card-info__desc sm:mb-[5.45rem]">
          <h2 className="single-card-info__desc-title">{SingleCardPageTr.desc}</h2>
      ...
      <Modal isActive={isModal} setIsActive={setIsModal}>
        ...
        <Link to="/auth/sign-in" className="w-full"><Button>{SingleCardPageTr.btn}</Button></Link>
        <div className="modal-content-no-auth">
					{SingleCardPageTr.notAcc}
          <Link to="/auth/sign-up" className="text-main">{SingleCardPageTr.register}</Link>
        </div>
      </Modal>
    </div>
  )
